...
class controllerAdmin extends modelAdmin
{
    ...
    public function pages_admin_controller($pages, $register, $role, $code, $search)
    {
        ...
        if ($_SESSION && isset($_SESSION['searchUser'])) {
            $clear_search = ($search == "");
            $search = mainModel::clean_string($_SESSION['searchUser']);

            if ($clear_search && $_SESSION['searchUser'] !== "" ) {
                $_SESSION['searchUser'] = "";
            }
        }
        ...
        if (isset($search) && $search != "") {
            $consulta = "SELECT SQL_CALC_FOUND_ROWS * FROM accounts a
                ...
                WHERE ((a.idAccount!='1') AND (accountFirstName like '%$search%' OR accountLastName like '%$search%' OR accountDni like '%$search%'))
        ...
        if ($total >= 1 && $pages <= $Npages) {
            ...
            if ($total >= 1) {
                $table .= '
                    <tr>
                        <td colspan="5"> 
                            <a href="'.SERVERURL.'admin/">Haga clic para recargar</a>
                        </td>   
                    </tr>';
            } else {
                $table .= '
                    <tr>
                        <td colspan="5"> No hay registros en esta página</td>
                    </tr>';
            }
        }
